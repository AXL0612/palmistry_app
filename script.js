  ...
  function renderResults(results, disclaimer, errors) {
    ...
    for (const key of ["sinh_dao", "su_nghiep", "tam_dao", "tri_dao"]) {
      ...
      if (!data) {
        ...
        fillEl.style.strokeDashoffset = String(GAUGE_CIRCUMFERENCE);
        continue;
      }

      scoreEl.textContent = `${data.score}%`;
      labelEl.textContent = data.nhan;
      ...
      // Đặt offset = full (rỗng) trước, rồi mới set giá trị thật ở frame sau
      // để CSS transition chạy mượt từ 0 -> điểm số thật.
      fillEl.style.strokeDashoffset = String(GAUGE_CIRCUMFERENCE);
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          const offset = GAUGE_CIRCUMFERENCE * (1 - data.score / 100);
